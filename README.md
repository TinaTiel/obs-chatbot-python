...
| ShowSource | Shows a scene item for a specified duration. Defaults to the item in the current scene unless parent scene is specified | `source` (list): List of scene items to show/hide, picks one from the list at random. | `scene` (string): The scene the scene item is nested in. Depth/nesting does not matter; if a scene is included in another scene, the item will still be shown/hidden. <br> `duration` (integer): Seconds to show the scene source. Permanent if not specified. |
