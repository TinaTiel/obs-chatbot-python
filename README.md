...
| ShowSource | Shows a scene item for a specified duration. Defaults to the item in the current scene unless parent scene is specified | `source` (string or list): The source to show/hide. If provided as a list, then `pick_from_group` is ignored and a scene is picked randomly from the specified list. | `scene` (string): The scene the scene item is nested in. Depth/nesting does not matter; if a scene is included in another scene, the item will still be shown/hidden. <br> `duration` (integer): Seconds to show the scene source. Permanent if not specified. <br> `pick_from_group` (true/false): If `true`, then it treats the specified source as a group, picking a child source from the group to show/hide <br> **If this doesn't behave correctly after adding/removes groups/sources in OBS** try restarting OBS to clear OBS's cache. |
